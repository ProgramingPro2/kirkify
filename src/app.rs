...
impl KirkifyApp {
    ...
    fn ensure_worker(&mut self, _ctx: &egui::Context) {
        ...
        let worker = {
            ...
            .unwrap_or_else(|| {
                // Fallback: parse from Error stack trace to find kirkify-{hash}.js
                ...
                if let Ok(stack_val) = js_sys::Reflect::get(&error, &JsValue::from_str("stack")) {
                    if let Some(stack) = stack_val.as_string() {
                        // Look for kirkify-{hash}.js in the stack
                        if let Some(start) = stack.find("kirkify-") {
