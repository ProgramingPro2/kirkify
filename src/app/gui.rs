                        ...
                        GuiMode::Transform => {
                            ...
                            ui.horizontal_wrapped(|ui| {
                                ...
                                let button_response = if !self.gui.has_obamified_once {
                                    ...
                                    let button = egui::Button::new("kirkify new image")
                                        .stroke(egui::Stroke::new(1.0, glow_color));
                                    ui.add(button)
                                } else {
                                    ui.button("kirkify new image")
                                };
                                ...
                                if button_response.clicked() {
                                    ...
                                    if let Some((ref img, ref settings)) = self.gui.saved_config {
                                        ...
                                        prompt_image(
                                            "choose image to kirkify",
                                            self,
                                            |name: String, mut img: SourceImg, app: &mut KirkifyApp| {
                        ...
                        |ui| {
                            ...
                            if change_source {
                                prompt_image(
                                    "choose image to kirkify",
                                    self,
                                    |_, mut img: SourceImg, app: &mut KirkifyApp| {
                                        img = ensure_reasonable_size(img);
...
fn prompt_image(
    title: &'static str,
    app: &mut KirkifyApp,
    callback: impl FnOnce(String, image::RgbImage, &mut KirkifyApp) + 'static,
) {
    #[cfg(target_arch = "wasm32")]
    {
        use wasm_bindgen_futures::spawn_local;
        let app_ptr: *mut KirkifyApp = app;
